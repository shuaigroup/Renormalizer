...
class Mpo(MatrixProduct):
    ...
    @classmethod
    def onsite(cls, mol_list: MolList, opera, dipole=False, nrdmol_idx_set=None,nrd=False):
        ...
        if nrd:
            ...
            for imol in range(nmols):
                ...
                for iph in range(len(mol_list[imol].dmrg_phs)):  #use iph instead of ph
                    ...
                    mo[0, :, :, 0] = phop["Iden"]
                    mo[1, :, :, 0] = mol_list[imol].dmrg_phs[iph].nac[1] * np.sqrt(0.5 * mol_list[imol].dmrg_phs[iph].omega[0]) * (phop["b"] - phop[r"b^\dagger"])
                    if imol != nmols - 1:
                        mo[-1, :, :, -1] = phop["Iden"]
                    if iph != nphss - 1:
                        #print(impo)
