...
def current_op(model, distance_matrix=None):
    ...
    for ham_op in ham_terms:
        ...
        for dof_idx, dof_name in enumerate(ham_op.dofs):
            site_idx = model.dof_to_siteidx[dof_name]
            if model.basis[site_idx].is_electron:
                e_idx = model.e_dofs.index(dof_name)
        ...
        if symbol1 == r"a^\dagger":
            factor = distance_matrix[e_idx1][e_idx2]
        else:
            factor = distance_matrix[e_idx2][e_idx1]
    ...
    j_oper = Mpo(model, holstein_current_terms)
    logger.info(f"current operator bond dim: {j_oper.bond_dims}")
    if len(peierls_current_terms) != 0:
        j_oper2  = Mpo(model, peierls_current_terms)
        logger.info(f"Peierls coupling induced current operator bond dim: {j_oper2.bond_dims}")
    else:
        j_oper2 = None
    
    return j_oper, j_oper2
...
class TransportKubo(TdMpsJob):
    ...
    def _construct_current_operator(self):
        self.j_oper, self.j_oper2 = current_op(self.model, self.distance_matrix)
