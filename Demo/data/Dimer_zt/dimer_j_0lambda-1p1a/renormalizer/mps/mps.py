...
def adaptive_tdvp(fun):
    ...
    def adaptive_fun(self: "Mps", mpo, evolve_target_t):
        ...
        while True:
            ...
            mps_half1 = fun(cur_mps, mpo, dt / 2).mps_normalize()
            mps_half2 = fun(mps_half1, mpo, dt / 2).mps_normalize()
            mps = fun(cur_mps, mpo, dt).mps_normalize()
...
class Mps(MatrixProduct):
    ...
    def add(self, other):
        if self.coeff != other.coeff:
            self = self.scale(self.coeff, inplace=True)
            self.coeff = 1
            other = other.scale(other.coeff)
            other.coeff = 1
        return super().add(other).normalize()
    ...
    def norm(self):
        ...
        '''
        return np.linalg.norm(self.coeff) * self.mps_norm

    @property
    def mps_norm(self) -> float:
        r''' the norm of the mps
        '''
    ...
    def normalize(self, norm=None):
        r''' normalize the total wavefunction
        
        Parameters
        ----------
        norm: norm of the total wavefunction. Default to None.
        ``norm = None`` is equal to `norm = self.norm`
        
        Returns
        -------
        ``self`` is overwritten. Mps part is normalized to 1. The length
        accumulates into ``self.coeff``.
        '''
        if norm is None:
            norm = self.norm
        
        new_mps = self.mps_normalize()
        new_mps.coeff *= norm / np.linalg.norm(self.coeff)
        
        return new_mps
    
    def mps_normalize(self):
        r''' normalize the mps part to 1
        
        Returns
        -------
        ``self`` is overwritten. Mps part is normalized to 1. 
        ``self.coeff`` is not modified.
        '''

        new_mps = self.scale(1.0 / self.mps_norm, inplace=True)
        return new_mps
    ...
    def expand_bond_dimension(self, hint_mpo=None, coef=1e-10,
        ...
        if m_target <= 0:
            return self.canonicalise().canonicalise().normalize()
        ...
        if hint_mpo is None:
            expander = self.__class__.random(self.model, self.qntot, m_target)
        else:
            ...
            if not self.use_dummy_qn and specific_guess:
                if self.is_mps:
            ...
            while True:
                ...
                lastone = (hint_mpo @ lastone).normalize(1)
        logger.debug(f"expander bond dimension: {expander.bond_dims}")
        self.compress_config.bond_dim_max_value += self.bond_dims_mean
        return (self + expander.scale(coef, inplace=True)).canonicalise().canonicalise().normalize()
    ...
    def evolve(self, mpo, evolve_dt) -> "Mps":
        ...
        if np.iscomplex(evolve_dt):
            # the total wavefunction is normalized to 1 in imaginary propagation
            new_mps.normalize(1)
        else:
            # the mps part is normalized to 1. the coeff is not changed in real time propagation.
            new_mps.mps_normalize()
        return new_mps
        ...
        if config.adaptive:
            ...
            while True:
                ...
                for idx, term in enumerate(termlist):
                    ...
                    del term
                new_mps1 = compressed_sum(scaled_termlist[:-1]).normalize(1)
    ...
    def calc_e_ph_static_correlation(self,intersite) -> np.ndarray:
        key = "e_ph_static_correlation"
        n_e = self.model.n_edofs
        e_dofs = self.model.e_dofs
        n_v = self.model.n_vdofs
        v_dofs = self.model.v_dofs
        if key not in self.model.mpos:
            mpos = []
            op_terms = []
            for idx, dof1 in enumerate(e_dofs):
                for v_idx, vdof in enumerate(v_dofs):
                    #logger.info(f"vdof: {vdof}  dof1: {dof1}  {intersite}")
                    if int(vdof.split('_')[1]) == (int(dof1.split('_')[1])+intersite):
                        #logger.info('click')
                        op_terms.append(Op(r"a^\dagger a x", [dof1, dof1, vdof]))
            mpo = Mpo(self.model, terms=op_terms)
            mpos.append(mpo)
            #self.model.mpos[key] = mpos
        else:
            mpos = self.model.mpos[key]
        
        return self.expectation(mpo)
    
    def calc_e_ph_static_correlation_2(self) -> np.ndarray:
        key = "e_ph_static_correlation"
        n_e = self.model.n_edofs
        e_dofs = self.model.e_dofs
        n_v = self.model.n_vdofs
        v_dofs = self.model.v_dofs
        v_per_e = int(np.floor(n_v / n_e))
        #e_ph_correlation_matrix = np.zeros((v_per_e,n_e))
        if key not in self.model.mpos:
            mpos= [[] for i in range(v_per_e)]
            for intersite in range(n_e):
                op_terms=[[] for i in range(v_per_e)]
                for idx, dof1 in enumerate(e_dofs):
                    for v_idx, vdof in enumerate(v_dofs):
                        #logger.info(f"vdof: {vdof}  dof1: {dof1}  {intersite}")
                        if int(vdof.split('_')[1]) == (int(dof1.split('_')[1])+intersite):
                            #logger.info('click')
                            op_terms[int(vdof.split('_')[-1])].append(Op(r"a^\dagger a x", [dof1, dof1, vdof]))
                for i_v in range(v_per_e):
                    mpos[i_v].append(Mpo(self.model,terms=op_terms[i_v]))
                    logger.info(f"intersite: {intersite} no_v: {i_v} length_mpos: {len(mpos[i_v])} {len(mpos)}")
            
            #self.model.mpos[key] = mpos
        else:
            mpos = self.model.mpos[key]
        e_ph_correlation_matrix = [[]]*v_per_e
        for i in range(v_per_e):
            e_ph_correlation_matrix[i] = self.expectations(mpos[i])

        return e_ph_correlation_matrix
    

    def calc_e_ph_static_correlation_3(self) -> np.ndarray:
        key = "e_ph_static_correlation"
        n_e = self.model.n_edofs
        e_dofs = self.model.e_dofs
        n_v = self.model.n_vdofs
        ...
        if key not in self.model.mpos:
            mpos= [[] for i in range(v_per_e)]
            op_terms=[[] for i in range(v_per_e)]
            e_ph_correlation_matrix = np.zeros((v_per_e,n_e,n_e))
            for idx, dof1 in enumerate(e_dofs):
                for v_idx, vdof in enumerate(v_dofs):
                    #logger.info(f"vdof: {vdof}  dof1: {dof1}  {intersite}")
                    op = Op(r"a^\dagger a x", [dof1, dof1, vdof])
                    tmp_mpo = Mpo(self.model,terms=op)
                    tmp_e_ph_corr = self.expectation(tmp_mpo)
                    e_ph_correlation_matrix[int(vdof.split('_')[-1]),int(dof1.split('_')[1]),int(vdof.split('_')[1])]= tmp_e_ph_corr
    ...
    def sum(self):
        # the sum of all coefficient
        # \sum_{\{\sigma\}} C_{\{\sigma\}}
        assert self.is_mps
        res = xp.ones(1)
        for ms in self:
            tmp = xp.sum(asxp(ms), axis=1)
            res = xp.tensordot(res,tmp,axes=(0,0))
        return res.item() * self.coeff
            
    def partial_sum(self, idx):
        assert self.is_mps
        
        res_L = xp.ones(1)
        for ims in range(idx):
            tmp = xp.sum(asxp(self[ims]), axis=1)
            res_L = xp.tensordot(res_L,tmp,axes=(0,0))
        
        res_R = xp.ones(1)
        for ims in range(self.site_num-1, idx,-1):
            tmp = xp.sum(asxp(self[ims]), axis=1)
            res_R = xp.tensordot(tmp,res_R,axes=(-1,0))

        res = oe.contract("a,abc,c->b", res_L, asxp(self[idx]), res_R, backend=OE_BACKEND)

        return asnumpy(res) * self.coeff
