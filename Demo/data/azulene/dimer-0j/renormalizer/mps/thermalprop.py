...
class ThermalProp(TdMpsJob):
    ...
    def __init__(
        ...
        self.specific_guess = specific_guess  # used in expand_bond_dimension
    ...
    def init_mps(self):
        ...
        if self.evolve_config.is_tdvp and self.auto_expand:
            self.init_mpdm = self.init_mpdm.expand_bond_dimension(self.h_mpo,
                    specific_guess=self.specific_guess)
    ...
    def evolve_prop(self, old_mpdm, evolve_dt):
        ...
        h_mpo = self.h_mpo - Mpo.identity(self.h_mpo.model).scale(self.energies[-1]) 
