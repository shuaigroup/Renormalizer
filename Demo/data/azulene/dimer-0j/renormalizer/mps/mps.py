        ...
        if key not in self.model.mpos:
            ...
            for intersite in range(n_e):
                ...
                for i in range(v_per_e):
                    mpos[i].append(Mpo(self.model,terms=op_terms[i]))
            
            self.model.mpos[key] = mpos
        else:
            mpos = self.model.mpos[key]
        e_ph_correlation_matrix = np.zeros(v_per_e)
        for i in range(v_per_e):
	    e_ph_correlation_matrix[i] = deque(self.expectations(mpos[i]))
