...
logger.info(f"s0 energy: {np.sum(w0)/2}")
logger.info(f"s0 energy: {e_ad+np.sum(w0)/2+np.sum(w1)/2}")
nmodes = len(w0)
...
exp_j = -1672*constant.cm2au
re_E = np.sum(w1**2*d1**2/2)
j_matrix[0,1] = -1*re_E*1e-10
j_matrix[1,0] = -1*re_E*1e-10

ham_terms = []

q_direction=[1]*nmodes
#negative_q_indice= np.array([170,172,177,178,180,187,189,190,191,192])-1
#for i in range(nmodes):
#    if i in negative_q_indice:
#        q_direction[i] = -1
#logger.info(f"q_direction: {q_direction}")


# 1mr s1
ex_order = np.argsort(w1)
logger.info(f'ex_order is {ex_order}')
...
for imol in range(nmols):
    ...
    scale_coeff_list =[]
    fname = "./par/midas/s1/"
    for midas_imode in range(nmodes):
        with open(fname+"savedir/prop_no_1.mop") as f:
            lines = f.readlines()
            scale_coeff = float(lines[8].split()[midas_imode])
            logger.info(f"scale_coeff:{scale_coeff},omega_cal:{np.sqrt(w1[ex_order[midas_imode]])}")
            scale_coeff_list.append(scale_coeff)
            poly = {}
            for line in lines:
                dof = line[line.find("(")+1:line.find(")")]
                if dof == f"Q{midas_imode}":
                    order = line[line.find("^")+1:line.find("(")]
                    poly[int(order)] = float(line.split()[0])
            logger.info(f"poly:{poly}")
    ...
    for midas_imode in range(nmodes):
        for key, value in poly_list[midas_imode].items():
            for k in range(key+1):
                if k!= key :
                    ham_terms.append(Op(f"a^\dagger a x^{key-k}", [f"ex_{imol}", f"ex_{imol}", f"e_{imol}_v_{ex_order[midas_imode]}"],
                                    factor=value * (scale_coeff_list[midas_imode]*q_direction[ex_order[midas_imode]])** key * comb(key,k) * d1[ex_order[midas_imode]]**k, qn=[1, -1, 0]))
                else:
                    ham_terms.append(Op(f"a^\dagger a", [f"ex_{imol}", f"ex_{imol}"],
                                    factor=value * (scale_coeff_list[midas_imode]*q_direction[ex_order[midas_imode]])** key * d1[ex_order[midas_imode]]**k, qn=[1, -1]))
...
for imol in range(nmols):
    for imode in range(nmodes):
        if w1[imode]*d1[imode]**2/2 >= 1:
            basis.append(ba.BasisSHO(f"e_{imol}_v_{imode}", w0[imode], 25))
        else:
            basis.append(ba.BasisSHO(f"e_{imol}_v_{imode}", w0[imode], 10))
...
#    nac_terms.append(Op("a", [f"ex_{imol}"],
#                        factor=1, qn=[-1]))
