...
class BraKetPairEmiFiniteT(BraKetPairBase):

    def calc_ft(self, factor):
        return np.conj(self.bra_mps.conj().dot(self.ket_mps)) / factor


class BraKetPairAbsFiniteT(BraKetPairBase):

    def calc_ft(self, factor):
        return self.bra_mps.conj().dot(self.ket_mps) / factor
...
class SpectraFiniteT(SpectraTdMpsJobBase):
    ...
    def exact_eiht(self, evolve_dt):
        if evolve_dt not in self._exact_eiht_cache:
            self._exact_eiht_cache[evolve_dt] = Mpo.exact_propagator(self.mol_list, 1.0j * evolve_dt,
                                                                     shift=self.gs_shift)
    ...
    def evolve_single_step(self, evolve_dt):
        ...
        return self.latest_mps.__class__(latest_bra_mpo, latest_ket_mpo, self.factor)
...
class SpectraEmiFiniteT(SpectraFiniteT):
    ...
    def init_mps(self):
        ...
        # e^{\-beta H/2} \Psi
        self.factor = ket_mpo.conj().dot(ket_mpo)
        ...
        return BraKetPairEmiFiniteT(bra_mpo, ket_mpo, self.factor)
...
class SpectraAbsFiniteT(SpectraFiniteT):
    ...
    def init_mps(self):
        ...
        bra_mpo = ket_mpo.copy()
        return BraKetPairAbsFiniteT(bra_mpo, ket_mpo, self.factor)
