...
class TestHybridAbs(unittest.TestCase):
    ...
    @unpack
    def test_hybrid_abs(self, algorithm, mol_list, std_fname, rtol):
...
class TestHybridEmi(unittest.TestCase):
    ...
    @unpack
    def test_hybrid_emi(self,algorithm, mol_list, std_fname, rtol):
        np.random.seed(0)
        # print "data", value
        if algorithm == 1:
            SpectraZeroT = SpectraOneWayPropZeroT
...
@ddt
class TestHybridExactEmi(unittest.TestCase):
    def test_Exact_Spectra_hybrid_TDDMRG_TDH(self):
        # print "data", value
        ...
        self.assertTrue(np.allclose(exact_emi.autocorr[:nsteps], std[:nsteps], rtol=1e-5))

@ddt
class TestHybridExactAbs(unittest.TestCase):
    @data(
            ["pure"],
            ["hybrid"])
    @unpack
    def test_1mol_Exact_Spectra_hybrid_TDDMRG_TDH(self, algorithm):
        ...
        self.assertTrue(np.allclose(exact_abs.autocorr[:nsteps], mol1_ZTabs_std[:nsteps],rtol=1e-3))
