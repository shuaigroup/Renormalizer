...
class TestZt(unittest.TestCase):
    ...
    def test(self, n_dmrg_phs):
        ...
        with open(os.path.join(cur_dir, "ZT_occ" + str(n_dmrg_phs) + ".npy"), 'rb') as f:
            std = np.load(f)
        self.assertTrue(np.allclose(occ, std, rtol=1e-2))
...
class TestFt(unittest.TestCase):
    ...
    @unpack
    def test_FT_dynamics_hybrid_TDDMRG_TDH(self, n_dmrg_phs):
        ...
        with open(os.path.join(cur_dir, "FT_occ"+str(n_dmrg_phs)+".npy"), 'rb') as f:
            std = np.load(f)
        self.assertTrue(np.allclose(occ[:, :nsteps], std[:, :nsteps], atol=1e-3))
