...
# eV
_j_matrix = np.array([[0.0, -0.1, -0.2], [-0.1, 0.0, -0.3], [-0.2, -0.3, 0.0]]) / constant.au2ev
...
hartree_ph_list = [Phonon(*args, hartree=True) for args in zip(omega, displacement, ph_phys_dim)]
hybrid_ph_list = [ph_list[1], hartree_ph_list[0]]

mol_list = MolList([Mol(elocalex, ph_list, dipole_abs)] * nmols, _j_matrix)
hartree_mol_list = MolList([Mol(elocalex, hartree_ph_list, dipole_abs)] * nmols, _j_matrix)
