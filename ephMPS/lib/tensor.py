...
# gpu version. currently not used.
def multi_tensor_contract_gpu(path, *operands):
    """
    ipath[0] is the index of the mat
    ipaht[1] is the contraction index
    oeprands is the arrays
    ...
    for ipath in path:
        ...
        tmpmat = pair_tensor_contract_gpu(
            operands[ipath[0][0]],
            input_str[0],
            operands[ipath[0][1]],
            input_str[1],
...
# gpu version...
import cupy as cp
def pair_tensor_contract_gpu(view_left, input_left, view_right, input_right, idx_removed):
    # Find indices to contract over
    left_pos, right_pos = (), ()
    for s in idx_removed:
        left_pos += (input_left.find(s),)
        right_pos += (input_right.find(s),)
    new_view = cp.tensordot(view_left, view_right, axes=(left_pos, right_pos))
